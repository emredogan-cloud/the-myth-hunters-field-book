# BOOK STATS — The Myth Hunter's Field Book

<!-- ÜRETİLMİŞTİR — 04_BUILD/update_docs.py · ELLE DÜZENLEMEYİN -->

> Kapı: `phase1` · Bu dosya **ölçümden üretilir**; hiçbir sayı elle yazılmaz.

## 1. Tek bakışta

| | Ölçülen | Hedef |
|---|---:|---:|
| Aday aktivite | **168** | ≥160 |
| Kilitli aktivite | **104** | 120 |
| Yazılmış aktivite | **104** | 120 |
| Düşürülmüş aday | **0** | — |
| Kültür | **22** | 22 |
| Bölge | **6** | 6 |
| Matris deliği (6×5) | **0** | 0 |
| Kısıt taraması | **168/168** | muafiyetsiz |
| Görsel öğe | **0** | ~150 |

## 2. Güvenlik sınıfı

| Sınıf | Aday | Oran | Hedef |
|---|---:|---:|---:|
| `safe` | 162 | %96.4 | ≥%90 |
| `safe-with-adult` | 6 | %3.6 | ≤%10 |
| `do-not-use` | 0 | %0.0 | 0 |

## 3. Devralma durumu

| Durum | Kayıt |
|---|---:|
| `inherited-provisional` | 28 |
| `inherited-verified` | 48 |
| `new-researched` | 0 |
| **Toplam** | **76** |
| **Doğrulanmış oran** | **%63.2** (Faz 1 ölçütü: doğrulanmış VEYA planlı) |

> Doğrulama KAYIT düzeyinde değil KULLANIM düzeyinde ilerler:
> bir kayıt, ondan CEVAP ÜRETEN bir sayfa yazıldığında
> doğrulanır. Bu yüzden oran aktivite üretimiyle birlikte
> yükselir ve Faz 2 sonunda %100 OLMASI BEKLENMEZ.
> Alan düzeyindeki kanıt: `01_SOURCE/research/*-revalidation.json`

## 4. Sayfa ve fiyat modeli

| | |
|---|---:|
| Aktivite sayfası (ölçülen ağırlıktan) | 104.0 |
| Ön madde | 8 |
| Bölge başına yapı sayfası | 2 |
| Final görev | 5 |
| Arka madde | 14 |
| **Model (forma hizalı)** | **144** |
| Yol haritası hedefi | 148 |
| Sapma | %-2.7 |
| Ciltsiz baskı maliyeti | 3.45 $ |
| **Ciltsiz telif** | **5.55 $** |
| Başabaş ACOS | %37.0 |

## 5. Bölge × tip matrisi

| Bölge | cipher | map | sort | observe | make | kota |
|---|---|---|---|---|---|---|
| north-ice | 7 | 5 | 10 | 7 | 5 | 24 |
| middle-sea | 7 | 4 | 7 | 6 | 4 | 20 |
| sun-savanna | 5 | 3 | 7 | 5 | 4 | 16 |
| monsoon | 9 | 5 | 9 | 6 | 5 | 24 |
| great-ocean | 7 | 4 | 5 | 6 | 4 | 20 |
| jaguar-condor | 6 | 3 | 5 | 5 | 3 | 16 |
| **minimum** | 3 | 2 | 4 | 2 | 2 | — |

