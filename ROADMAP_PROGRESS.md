# ROADMAP PROGRESS — The Myth Hunter's Field Book

<!-- ÜRETİLMİŞTİR — 04_BUILD/update_docs.py · ELLE DÜZENLEMEYİN -->

> Kapı: `release`

---

## Faz durumu

| Faz | Ad | Durum | Kapı | Dal | Etiket |
|---|---|---|---|---|---|
| **0** | Bootstrap | ✅ **TAMAM** | `phase0` | `main` | — |
| **1** | Devralma mimarisi, taksonomi, yaş çerçevesi | ✅ **TAMAM** | `phase1` | `faz/1-devralma` | v0.1.0 |
| **2** | Pilot: bir bölge + çocuk saha testi | ✅ **TAMAM** | `phase2` | `faz/2-pilot` | v0.2.0 |
| **3** | Bölge bloğu I — üç bölge | ✅ **TAMAM** | `phase3` | `faz/3-blok-1` | v0.3.0 |
| **4** | Bölge bloğu II + final görev | ✅ **TAMAM** | `phase4` | `faz/4-blok-2` | v0.4.0 |
| **5** | Editoryal yakınsama + sayfa tasarımı | ✅ **TAMAM** | `phase5` | `faz/5-yakinsama` | v0.5.0 |
| **6** | Nihai üretim + KDP paketi | ✅ **TAMAM** | `release` | `faz/6-uretim` | v1.0.0 |

---

## Ölçülen ilerleme

| | Ölçülen | Hedef |
|---|---:|---:|
| Aday aktivite | **168** | ≥160 |
| Kilitli aktivite | **120** | 120 |
| Yazılmış aktivite | **120** | 120 |
| Devralınan kayıt | **76** | — |
| Kültür | **22** | 22 |
| Bölge (tanımlı) | **6** | 6 |
| **Bölge (yazılmış)** | **6** | 6 |
| Sayfa basım maddesi (`pagePrints`) | **570** | — |
| Görsel şartnamesi | **120** | ~150 |
| Görsel varlık (üretilmiş) | **0** | ~150 |
| Kelime | **9.597** | ~22.000 |

---

## Sonraki izinli eylem

> Proje burada biter. **AJAN DURUR.**

